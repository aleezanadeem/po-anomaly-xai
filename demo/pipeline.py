...
class ScoreProvider:
    """Demo version: always fallback to precomputed scores (CSV)."""
    def __init__(self, cfg: dict, features_df: pd.DataFrame):
        self.cfg = cfg
        self.features_df = features_df
        self._scores_lookup = None
        self.feat_names: List[str] = []  # not used in demo
...
class StreamSimulator:
    ...
    def stats(self) -> dict:
        ...
        return {
            ...
            "now": self.now,
            "flags_df": log_df.sort_values("time", ascending=False).head(1000),  # show last 1000 flags
        }
